...
const api = axios.create({
  baseURL: "http://192.168.132.232:4000/api", // Base URL de tu API
});
    ...
    if (error.response?.status === 401 && !originalRequest._retry) {
      ...
      if (refreshToken) {
        if (!isRefreshing) {
          ...
          try {
            const { data } = await axios.post("http://192.168.132.232            npx expo start --clear:4000/api/users/auth/refresh", {
              refreshToken,
            });
